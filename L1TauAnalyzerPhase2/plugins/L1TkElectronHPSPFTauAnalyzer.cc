// Class:      L1TkElectronHPSPFTauAnalyzer
//
...
class L1TkElectronHPSPFTauAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit L1TkElectronHPSPFTauAnalyzer(const edm::ParameterSet&);
...
private:
  ...
  Int_t           lumi_;
  float MC_weight_;
  std::vector<float> genElectronPt_;
  std::vector<float> genElectronEta_;
  std::vector<float> genElectronPhi_;
  std::vector<int> genElectronCharge_;
  ...
  std::vector<float> l1TkElectronPt_;
  std::vector<float> l1TkElectronEta_;
  std::vector<float> l1TkElectronPhi_;
  std::vector<int> l1TkElectronCharge_;
  ...
  std::vector<float> single_l1TkElectronPt_;
  std::vector<float> single_l1TkElectronEta_;
  std::vector<float> single_l1TkElectronPhi_;
  std::vector<int> single_l1TkElectronCharge_;
  ...
  */
  ...
  TFile* histRootFile_;
  TH1F* hist_genElectronPt_;
  TH1F* hist_genElectronEta_;
  TH1F* hist_genElectronPhi_;
  TH1F* hist_genTauPt_;
  ...
  TH1F* hist_l1TkElectronPt_;
  TH1F* hist_l1TkElectronEta_;
  TH1F* hist_l1TkElectronPhi_;
  ...
  bool debug_;
  bool isReco_;
  double min_electron_pt_;
  double min_tau_pt_;
  double max_eta_;
  typedef std::vector<int> vint;
  vint electronPDGIds_;
  ...
  edm::EDGetTokenT<reco::GenParticleCollection>    genElectronToken_;
  ...
  edm::EDGetTokenT<l1t::L1TkElectronHPSPFTauCollection>l1TkElectronPFTauToken_;
  edm::EDGetTokenT<l1t::L1TkElectronParticleCollection>l1TkElectronToken_;
...
L1TkElectronHPSPFTauAnalyzer::L1TkElectronHPSPFTauAnalyzer(const edm::ParameterSet& iConfig)
  : debug_          (iConfig.getUntrackedParameter<bool>("debug", false))
  , isReco_         (iConfig.getUntrackedParameter<bool>("isReco", false))
  , min_electron_pt_         (iConfig.getUntrackedParameter<double>("min_electron_pt", 10))
  , min_tau_pt_         (iConfig.getUntrackedParameter<double>("min_tau_pt", 20))
  , max_eta_        (iConfig.getUntrackedParameter<double>("max_eta", 2.4))
  , electronPDGIds_     (iConfig.getUntrackedParameter<vint>("electronPDGIds"))
  ...
  , genElectronToken_    (consumes<reco::GenParticleCollection> (iConfig.getParameter<edm::InputTag>("genElectronToken")))
  ...
  , l1TkElectronPFTauToken_(consumes<l1t::L1TkElectronHPSPFTauCollection>(iConfig.getParameter<edm::InputTag>("l1TkElectronPFTauToken")))
  , l1TkElectronToken_(consumes<l1t::L1TkElectronParticleCollection>(iConfig.getParameter<edm::InputTag>("l1TkElectronToken")))
...
L1TkElectronHPSPFTauAnalyzer::~L1TkElectronHPSPFTauAnalyzer()
{
...
void
L1TkElectronHPSPFTauAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  Initialize();
  ...
  if(debug_){
   ...
   edm::Handle<reco::GenParticleCollection> genElectronHandle;
   iEvent.getByToken(genElectronToken_,         genElectronHandle);
   ...
   edm::Handle<l1t::L1TkElectronHPSPFTauCollection>  l1TkElectronPFTauHandle;
   iEvent.getByToken(l1TkElectronPFTauToken_,        l1TkElectronPFTauHandle);

   edm::Handle<l1t::L1TkElectronParticleCollection>  l1TkElectronHandle;
   iEvent.getByToken(l1TkElectronToken_,        l1TkElectronHandle);

   for(auto genElectron : *genElectronHandle){
     if ( electronPDGIds_.size() > 0 ){
       bool isSelected = false;
       for ( auto PDGId : electronPDGIds_ ){
	 if ( genElectron.pdgId() == PDGId ) isSelected = true;
       }
       if ( !isSelected ) continue;
     }
     if (fabs(genElectron.eta())>max_eta_)
       continue;
     if (fabs(genElectron.pt())<min_electron_pt_)
       continue;

     genElectronPt_.push_back(genElectron.pt());
     genElectronEta_.push_back(genElectron.eta());
     genElectronPhi_.push_back(genElectron.phi());
     genElectronCharge_.push_back(genElectron.charge());

     hist_genElectronPt_->Fill(genElectron.pt());
     hist_genElectronEta_->Fill(genElectron.eta());
     hist_genElectronPhi_->Fill(genElectron.phi());

     if(debug_){
       std::cout<<" GenElectron pt "<<genElectron.pt()<<" eta "<< genElectron.eta()<<" phi "<< genElectron.phi()<<" charge "<< genElectron.charge()<<std::endl;
     }
   }
   ...
   for(auto genTau : *genTauHandle){
     if (fabs(genTau.eta())>max_eta_)
       continue;
     //if (fabs(genTau.pt())<min_electron_pt_)
     //continue;
   ...
   for(auto l1TkElectron : *l1TkElectronHandle){
     if(fabs(l1TkElectron.pt())<min_electron_pt_)
       continue;
     if(fabs(l1TkElectron.eta())>max_eta_)
       continue;

     single_l1TkElectronPt_.push_back(l1TkElectron.pt());
     single_l1TkElectronEta_.push_back(l1TkElectron.eta());
     single_l1TkElectronPhi_.push_back(l1TkElectron.phi());
     single_l1TkElectronCharge_.push_back(l1TkElectron.charge());

     if(debug_){
       std::cout<<" L1TkElectron pt "<<l1TkElectron.pt()<<" eta "<< l1TkElectron.eta()<<" phi "<< l1TkElectron.phi()<<" charge "<< l1TkElectron.charge()<<std::endl;
     }

   }


   for(auto l1TkElectronPFTau : *l1TkElectronPFTauHandle){

     if(fabs(l1TkElectronPFTau.L1TkElectronParticle()->pt())<min_electron_pt_)
       continue;
     if(fabs(l1TkElectronPFTau.L1TkElectronParticle()->eta())>max_eta_)
       continue;
     if(fabs(l1TkElectronPFTau.L1HPSPFTau()->pt())<min_tau_pt_)
       continue;
     if(fabs(l1TkElectronPFTau.L1HPSPFTau()->eta())>max_eta_)
       continue;

     l1TkElectronPt_.push_back(l1TkElectronPFTau.L1TkElectronParticle()->pt());
     l1TkElectronEta_.push_back(l1TkElectronPFTau.L1TkElectronParticle()->eta());
     l1TkElectronPhi_.push_back(l1TkElectronPFTau.L1TkElectronParticle()->phi());
     l1TkElectronCharge_.push_back(l1TkElectronPFTau.L1TkElectronParticle()->charge());

     l1PFTauPt_.push_back(l1TkElectronPFTau.L1HPSPFTau()->pt());
     l1PFTauEta_.push_back(l1TkElectronPFTau.L1HPSPFTau()->eta());
     l1PFTauPhi_.push_back(l1TkElectronPFTau.L1HPSPFTau()->phi());
     l1PFTauCharge_.push_back(l1TkElectronPFTau.L1HPSPFTau()->charge());
     l1PFTauType_.push_back(l1TkElectronPFTau.L1HPSPFTau()->tauType());
     l1PFTauIso_.push_back(l1TkElectronPFTau.L1HPSPFTau()->sumChargedIso());
     l1PFTauTightIso_.push_back(l1TkElectronPFTau.L1HPSPFTau()->passTightIso());
     l1PFTauMediumIso_.push_back(l1TkElectronPFTau.L1HPSPFTau()->passMediumIso());
     l1PFTauLooseIso_.push_back(l1TkElectronPFTau.L1HPSPFTau()->passLooseIso());
     l1PFTauVLooseIso_.push_back(l1TkElectronPFTau.L1HPSPFTau()->passVLooseIso());
     double l1PFTauZ = 1000;
     if ( l1TkElectronPFTau.L1HPSPFTau()->leadChargedPFCand().isNonnull() && l1TkElectronPFTau.L1HPSPFTau()->leadChargedPFCand()->pfTrack().isNonnull())
       {
         l1PFTauZ = l1TkElectronPFTau.L1HPSPFTau()->leadChargedPFCand()->pfTrack()->vertex().z();
       }
     l1PFTauZ_.push_back(l1PFTauZ);

     if(l1TkElectronPFTau.L1HPSPFTau()->pt()!=0){
       if(l1TkElectronPFTau.L1HPSPFTau()->sumChargedIso()/l1TkElectronPFTau.L1HPSPFTau()->pt() < 0.40){
	 l1PFTauVLooseRelIso_.push_back(true);
       }
       else{
	 l1PFTauVLooseRelIso_.push_back(false);
       }
       if(l1TkElectronPFTau.L1HPSPFTau()->sumChargedIso()/l1TkElectronPFTau.L1HPSPFTau()->pt() < 0.20){
	 l1PFTauLooseRelIso_.push_back(true);
       }
       else{
	 l1PFTauLooseRelIso_.push_back(false);
       }
       if(l1TkElectronPFTau.L1HPSPFTau()->sumChargedIso()/l1TkElectronPFTau.L1HPSPFTau()->pt() < 0.10){
	 l1PFTauMediumRelIso_.push_back(true);
       }
       else{
	 l1PFTauMediumRelIso_.push_back(false);
       }
       if(l1TkElectronPFTau.L1HPSPFTau()->sumChargedIso()/l1TkElectronPFTau.L1HPSPFTau()->pt() < 0.05){
     ...
     if(debug_){
       std::cout<<"l1Tk Electron Pt "<<l1TkElectronPFTau.L1TkElectronParticle()->pt()<<std::endl;
       std::cout<<"l1PF Tau Pt "<<l1TkElectronPFTau.L1HPSPFTau()->pt()<<std::endl;
     }
   }
...
void L1TkElectronHPSPFTauAnalyzer::Initialize() {
  indexevents_ = 0;
  runNumber_ = 0;
  lumi_ = 0;
  MC_weight_ = 1;
  genElectronPt_ .clear();
  genElectronEta_ .clear();
  genElectronPhi_ .clear();
  genElectronCharge_ .clear();
  genTauPt_ .clear();
  genTauEta_ .clear();
  genTauPhi_ .clear();
  genTauCharge_ .clear();
  ...
  l1TkElectronPt_ .clear();
  l1TkElectronEta_ .clear();
  l1TkElectronPhi_ .clear();
  l1TkElectronCharge_ .clear();
  l1PFTauPt_ .clear();
  l1PFTauEta_ .clear();
  l1PFTauPhi_ .clear();
  ...
  l1VertexN_ = 0;
  recoVertexN_ = 0;
  single_l1TkElectronPt_ .clear();
  single_l1TkElectronEta_ .clear();
  single_l1TkElectronPhi_ .clear();
  single_l1TkElectronCharge_ .clear();
...
void
L1TkElectronHPSPFTauAnalyzer::beginJob()
{
  ...
  tree_ -> Branch("genElectronPt",  &genElectronPt_);
  tree_ -> Branch("genElectronEta", &genElectronEta_);
  tree_ -> Branch("genElectronPhi", &genElectronPhi_);
  tree_ -> Branch("genElectronCharge", &genElectronCharge_);
  ...
  tree_ -> Branch("l1TkElectronPt",  &l1TkElectronPt_);
  tree_ -> Branch("l1TkElectronEta", &l1TkElectronEta_);
  tree_ -> Branch("l1TkElectronPhi", &l1TkElectronPhi_);
  tree_ -> Branch("l1TkElectronCharge", &l1TkElectronCharge_);
  ...
  tree_ -> Branch("single_l1TkElectronPt",  &single_l1TkElectronPt_);
  tree_ -> Branch("single_l1TkElectronEta", &single_l1TkElectronEta_);
  tree_ -> Branch("single_l1TkElectronPhi", &single_l1TkElectronPhi_);
  tree_ -> Branch("single_l1TkElectronCharge", &single_l1TkElectronCharge_);
  ...
  hist_genElectronPt_ = new TH1F("genElectronPt","genElectronPt", 100, 0., 1000.);
  hist_genElectronEta_ = new TH1F("genElectronEta","genElectronEta",50, -3., 3.);
  hist_genElectronPhi_ = new TH1F("genElectronPhi","genElectronPhi",50, -3., 3.);
  ...
  hist_l1TkElectronPt_ = new TH1F("l1TkElectronPt","l1TkElectronPt", 100, 0., 1000.);
  hist_l1TkElectronEta_ = new TH1F("l1TkElectronEta","l1TkElectronEta",50, -3., 3.);
  hist_l1TkElectronPhi_ = new TH1F("l1TkElectronPhi","l1TkElectronPhi",50, -3., 3.);
  ...
  if(fillBDT_)
    {
      ...
      treeBDT_ -> Branch("genElectronPt",  &genElectronPt_);
      treeBDT_ -> Branch("genElectronEta", &genElectronEta_);
      treeBDT_ -> Branch("genElectronPhi", &genElectronPhi_);
      treeBDT_ -> Branch("genElectronCharge", &genElectronCharge_);
      ...
      treeBDT_ -> Branch("l1TkElectronPt",   &l1TkElectronPt_);
      treeBDT_ -> Branch("l1TkElectronEta",  &l1TkElectronEta_);
      treeBDT_ -> Branch("l1TkElectronPhi", &l1TkElectronPhi_);
      treeBDT_ -> Branch("l1TkElectronCharge", &l1TkElectronCharge_);
...
void
L1TkElectronHPSPFTauAnalyzer::endJob()
{
  if(fillBDT_)
    {
      bdtRootFile_->cd();
  ...
  if(createHistRoorFile_){
    ...
    hist_genElectronPt_->Write();
    hist_genElectronEta_->Write();
    hist_genElectronPhi_->Write();
    ...
    hist_l1TkElectronPt_->Write();
    hist_l1TkElectronEta_->Write();
    hist_l1TkElectronPhi_->Write();
...
DEFINE_FWK_MODULE(L1TkElectronHPSPFTauAnalyzer);
